...
const SalonDetailsPage: React.FC = () => {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:8080/owner/salon/${salonId}?email=${encodeURIComponent(
        email
      )}`
    )
      .then((response) => {
  ...
  const handleOfferSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSalon((prevSalon) =>
        prevSalon
          ? { ...prevSalon, offerDto: [...prevSalon.offerDto, newOffer] }
          : prevSalon
      );
        ...
        <ul className="offer-list">
          {salon.offerDto.map((offer) => (
            <li key={offer.id} className="offer-item">
              <strong>{offer.name}</strong>: {offer.description} - {offer.price}{" "}
              PLN ({offer.duration})
            </li>
          ))}
        </ul>
